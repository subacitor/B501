...
(function ($) {
					...
					for (let e = 0; e <= 12; e++) {
						if (monthOption == e) {
							for (let i = 0; i < data.length; i++) {
								if (monthOption == data[i].month - 1) {
									$('td', frame).filter(function () { return $(this).text() == data[i].day }).addClass('today').append(" Đã mượn");
								}
							}
						}
					}
	...
	function triggerSelectEvent(event) {
		...
		// xử lý ngày tháng năm khi đã book lịch!


		// if ((date.getFullYear()) == dateSelected.year) {
		// 	if ((date.getMonth() + 1) == dateSelected.month) {
		// 		if (date.getDate() == dateSelected.day) {
		// 			console.log("work!");
		// 		}
		// 	}
		// }
